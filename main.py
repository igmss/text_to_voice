...
# Import our TTS system components
...
def get_system_info():
    ...
    return jsonify({
        'system': 'Egyptian Arabic TTS Voice Over System',
        'version': '1.0.0',
        'capabilities': {
...
def serve(path):
    ...
    if path != "" and os.path.exists(os.path.join(static_folder_path, path)):
        ...
        if os.path.exists(index_path):
            ...
            return jsonify({
                'message': 'Egyptian Arabic TTS Voice Over API',
                'version': '1.0.0',
                'endpoints': {
                    'health': '/api/health',
...
if __name__ == '__main__':
    print("🎙️ Starting Egyptian Arabic TTS Voice Over API Server")
    print(f"📁 Temporary audio directory: {TEMP_AUDIO_DIR}")
